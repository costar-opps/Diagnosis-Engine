...
public class AiOpsService {
    ...
    /**
     * 执行 AI Ops 告警分析流程，并在每个 Graph 节点产出时回调进度
     *
     * @param chatModel        大模型实例
     * @param toolCallbacks    工具回调数组
     * @param progressListener 节点级进度回调，传 null 时退化为阻塞式 invoke
     * @return 分析结果状态
     * @throws GraphRunnerException 如果 Agent 执行失败
     */
    ...
    public Optional<OverAllState> executeAiOpsAnalysis(DashScopeChatModel chatModel, ToolCallback[] toolCallbacks,
        ...
        ReactAgent executorAgent = buildExecutorAgent(chatModel, toolCallbacks, progressInterceptor);
    ...
    private ReactAgent buildPlannerAgent(DashScopeChatModel chatModel, ToolCallback[] toolCallbacks,
        ...
        if (progressInterceptor != null) {
            builder.interceptors(progressInterceptor);
        }
    ...
    private String buildPlannerPrompt() {
        return """
                ...
                5. 本环境监控的是 local-host（开发者本机），Prometheus 告警与 CLS 日志中的 service 字段均为 local-host。
                %s
                ...
                **重要提醒**：
                ...
                """.formatted(clsProperties.buildMcpQueryInstructions());
    }
