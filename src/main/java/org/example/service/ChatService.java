...
import org.example.config.ClsMcpToolCallbackDecorator;
...
public class ChatService {
    ...
    public String buildSystemPrompt(List<Map<String, String>> history) {
        ...
        if (!clsProperties.isMockEnabled()) {
            systemPromptBuilder.append("当用户需要查询腾讯云日志时，请调用腾讯云 MCP 服务查询。");
            systemPromptBuilder.append(clsProperties.buildMcpQueryInstructions());
            systemPromptBuilder.append("\n");
        } else {
            systemPromptBuilder.append("当用户需要查询日志时，使用 queryLogs 工具（Mock 模式）。\n");
        }
    ...
    public ToolCallback[] getToolCallbacks() {
        ...
        for (int i = 0; i < callbacks.length; i++) {
            wrapped[i] = new ClsMcpToolCallbackDecorator(callbacks[i], clsProperties);
        }
        return wrapped;
    }
