...
class SuperBizAgentApp {
    constructor() {
        ...
        this.LAST_AIOPS_STORAGE_KEY = 'superbizagent_last_aiops';
        this.DE_CURRENT_PROJECT_KEY = 'diagnosis_engine_current_project';
    ...
    initializeElements() {
        ...
        this.aiOpsMonitorNavBtn = document.getElementById('aiOpsMonitorNavBtn');
        this.chatView = document.getElementById('chatView');
        this.aiopsMonitorView = document.getElementById('aiopsMonitorView');
        this.aiopsMonitorContent = document.getElementById('aiopsMonitorContent');
        this.aiopsMonitorMeta = document.getElementById('aiopsMonitorMeta');
        this.aiopsMonitorDashboard = document.getElementById('aiopsMonitorDashboard');
        this.aiopsMonitorReport = document.getElementById('aiopsMonitorReport');
        this.backToChatBtn = document.getElementById('backToChatBtn');
        this.runAiOpsFromMonitorBtn = document.getElementById('runAiOpsFromMonitorBtn');
        this.loadCurrentMonitorBtn = document.getElementById('loadCurrentMonitorBtn');

        // 诊断控制台元素
        this.deProjectList = document.getElementById('deProjectList');
        this.deProjectSelect = document.getElementById('deProjectSelect');
        this.deModeBadge = document.getElementById('deModeBadge');
        this.deRefreshBtn = document.getElementById('deRefreshBtn');
        this.dePlatformBadge = document.getElementById('dePlatformBadge');
        this.deProjectBadge = document.getElementById('deProjectBadge');
        this.deProjectTitleName = document.getElementById('deProjectTitleName');
        this.deProjectBody = document.getElementById('deProjectBody');
        this.deDiagnosisStatus = document.getElementById('deDiagnosisStatus');
        this.deRunDiagnosisBtn = document.getElementById('deRunDiagnosisBtn');
        this.deReportBadge = document.getElementById('deReportBadge');
        this.deReportMeta = document.getElementById('deReportMeta');
        this.deTraceBar = document.getElementById('deTraceBar');
        this.deReportActions = document.getElementById('deReportActions');
    ...
    bindEvents() {
        ...
        if (this.aiOpsMonitorNavBtn) {
            this.aiOpsMonitorNavBtn.addEventListener('click', () => this.switchMainView('aiops-monitor'));
        }

        if (this.backToChatBtn) {
            this.backToChatBtn.addEventListener('click', () => this.switchMainView('chat'));
        }

        if (this.runAiOpsFromMonitorBtn) {
            this.runAiOpsFromMonitorBtn.addEventListener('click', () => {
                this.switchMainView('chat');
                this.triggerAIOps();
            });
        }

        if (this.loadCurrentMonitorBtn) {
            this.loadCurrentMonitorBtn.addEventListener('click', () => this.loadDashboardOnlyToMonitor());
        }

        if (this.deRefreshBtn) {
            this.deRefreshBtn.addEventListener('click', () => this.refreshAiOpsMonitorData());
        }

        if (this.deProjectSelect) {
            this.deProjectSelect.addEventListener('change', () => this.setCurrentProject(this.deProjectSelect.value));
        }

        if (this.deRunDiagnosisBtn) {
            this.deRunDiagnosisBtn.addEventListener('click', () => this.runProjectDiagnosis());
        }
    ...
    updateUI() {
        ...
        if (this.messageInput) {
            ...
            this.messageInput.placeholder = '问问 diagnosis-engine 诊断助手';
        }
    }
    ...
    async sendAIOpsRequest(loadingMessageElement) {
        try {
            ...
            const renderLive = () => {
                ...
                const lines = ['🔎 平台级诊断进行中'];
                lines.push.apply(lines, milestones);
                if (liveStatus && milestones[milestones.length - 1] !== '· ' + liveStatus) {
                    lines.push('⏳ ' + liveStatus);
                }
                this.updateAIOpsStreamContent(loadingMessageElement, lines.join('\n'));
            };

            // 用显式标记区分"后端回报的错误"与"这一行不是 JSON"，
            // 靠错误文案关键字判断会随文案改动而失效，导致错误被当正文吞掉
            const sseError = (text) => {
                const err = new Error(text || '智能运维分析失败');
                err.fromSse = true;
                return err;
            };
            ...
            const applyProgress = (text) => {
                if (!text) return;
                // 已完成的工具调用留作里程碑，"正在调用"和心跳只刷新末尾状态行
                if (text.indexOf('已返回') >= 0 || text.indexOf('调用失败') >= 0) {
                ...
                while (true) {
                    ...
                    if (done) {
                        ...
                        if (fullResponse) {
                            ...
                            // 后端异常断流：没有正文也没有结束标记，不能让界面一直停在 loading
                            throw new Error('智能运维分析连接意外中断，未收到任何报告内容');
                        }
                        break;
                    }
                                    ...
                                    for (const jsonStr of matches) {
                                        try {
                                            ...
                                            if (sseMessage.type === 'content') {
                                                ...
                                                throw sseError(sseMessage.data);
                                            }
                                        } catch (e) {
                                            if (e.fromSse) throw e;
    ...
    updateAIOpsStreamContent(messageElement, content) {
        ...
        if (messageContentWrapper) {
            ...
            // 流式显示时使用纯文本，需要 pre-wrap 才能保住换行
            messageContent.classList.add('de-stream-text');
            messageContent.textContent = content;
            this.scrollToBottom();
        }
    }
    ...
    updateAIOpsMessage(messageElement, response, details) {
        ...
        // 最终报告走 Markdown 渲染，不再需要流式纯文本的换行保留
        messageContent.classList.remove('de-stream-text');
        ...
        // 持久化并更新独立监控面板
        this.persistAndShowAiOpsResult(response);
    ...
    ensureAiOpsDashboardShell(messageElement) {
        if (!messageElement) return null;
        const wrapper = messageElement.querySelector('.message-content-wrapper');
        if (!wrapper) return null;

        let dashboard = wrapper.querySelector('.aiops-dashboard');
        if (!dashboard) {
            dashboard = document.createElement('div');
            dashboard.className = 'aiops-dashboard';
            wrapper.insertBefore(dashboard, wrapper.firstChild);
        }
        return dashboard;
    }

    showDashboardLoading(messageElement) {
        const dashboard = this.ensureAiOpsDashboardShell(messageElement);
        if (!dashboard) return;
        dashboard.innerHTML = `
            <div class="aiops-dashboard-loading">
                <div class="loading-spinner-icon" style="display:inline-block;margin-bottom:12px;">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8z" fill="currentColor" opacity="0.2"/>
                        <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10c1.54 0 3-.36 4.28-1l-1.5-2.6C13.64 19.62 12.84 20 12 20c-4.41 0-8-3.59-8-8s3.59-8 8-8c.84 0 1.64.38 2.18 1l1.5-2.6C13 2.36 12.54 2 12 2z" fill="#1a73e8"/>
                    </svg>
                </div>
                <div>正在加载监控可视化数据...</div>
            </div>`;
    ...
    async fetchAiOpsDashboard(rangeMinutes = 60) {
        const response = await fetch(`${this.apiBaseUrl}/monitoring/aiops-dashboard?rangeMinutes=${rangeMinutes}`);
        if (!response.ok) {
            throw new Error(`加载仪表盘失败: HTTP ${response.status}`);
        }
        return response.json();
    }

    destroyAiOpsCharts(messageElement) {
        if (!messageElement || !messageElement._aiopsCharts) return;
        messageElement._aiopsCharts.forEach(chart => {
            try { chart.destroy(); } catch (e) { /* ignore */ }
        });
        messageElement._aiopsCharts = [];
    }

    formatAlertState(state) {
        const map = { firing: '触发中', pending: '待触发', inactive: '已恢复' };
        return map[state] || state || '未知';
    }

    renderAlertCards(alerts) {
        if (!alerts || alerts.length === 0) {
            return '<div class="aiops-alert-empty">✅ 当前无活跃 Prometheus 告警</div>';
        }
        return alerts.map(alert => {
            const stateClass = alert.state === 'pending' ? 'pending' : (alert.state === 'inactive' ? 'inactive' : '');
            const badgeClass = alert.state || 'firing';
            return `
                <div class="aiops-alert-card ${stateClass}">
                    <div class="aiops-alert-card-header">
                        <span class="aiops-alert-name">${this.escapeHtml(alert.alert_name)}</span>
                        <span class="aiops-alert-badge ${badgeClass}">${this.escapeHtml(this.formatAlertState(alert.state))}</span>
                    </div>
                    <div class="aiops-alert-time">
                        <strong>首次触发：</strong>${this.escapeHtml(alert.active_at_display || alert.active_at || '-')}
                        &nbsp;|&nbsp;
                        <strong>持续：</strong>${this.escapeHtml(alert.duration || '-')}
                    </div>
                    <div class="aiops-alert-time">
                        <strong>服务：</strong>${this.escapeHtml(alert.service || 'local-host')}
                        &nbsp;|&nbsp;
                        <strong>级别：</strong>${this.escapeHtml(alert.severity || 'warning')}
                    </div>
                    <div class="aiops-alert-summary">${this.escapeHtml(alert.summary || alert.description || '')}</div>
                </div>`;
        }).join('');
    }

    renderGaugeCards(currentMetrics, thresholds) {
        const items = [
            { key: 'cpu', label: 'CPU 使用率', unit: '%', color: '#1a73e8' },
            { key: 'memory', label: '内存使用率', unit: '%', color: '#9334e6' },
            { key: 'disk', label: '磁盘使用率', unit: '%', color: '#188038' }
        ];
        return items.map(item => {
            const value = currentMetrics ? currentMetrics[item.key] : 0;
            const threshold = thresholds ? thresholds[item.key] : 100;
            const isWarn = value >= threshold;
            return `
                <div class="aiops-gauge-card ${isWarn ? 'warn' : ''}">
                    <div class="aiops-gauge-label">${item.label}</div>
                    <div class="aiops-gauge-value ${isWarn ? 'warn' : ''}">${Number(value).toFixed(1)}${item.unit}</div>
                    <div class="aiops-gauge-threshold">告警阈值 ${threshold}${item.unit}</div>
                </div>`;
        }).join('');
    }

    buildMetricChart(canvas, label, points, threshold, color) {
                    ...
                    tooltip: {
                        callbacks: {
                            title: (items) => {
                                ...
                                return (points && points[idx]) ? points[idx].time : '';
                            },
                            label: (ctx) => {
                                if (ctx.dataset.label === '告警阈值') {
                                    return `阈值: ${threshold}%`;
                                }
    ...
    renderAiOpsCharts(messageElement, data) {
        this.destroyAiOpsCharts(messageElement);
        if (!messageElement._aiopsCharts) {
            messageElement._aiopsCharts = [];
        }

        const chartConfigs = [
            { id: 'aiops-cpu-chart', key: 'cpu', label: 'CPU', color: '#1a73e8', threshold: data.thresholds.cpu },
            { id: 'aiops-memory-chart', key: 'memory', label: '内存', color: '#9334e6', threshold: data.thresholds.memory },
            { id: 'aiops-disk-chart', key: 'disk', label: '磁盘', color: '#188038', threshold: data.thresholds.disk }
        ];

        chartConfigs.forEach(cfg => {
            const canvas = messageElement.querySelector('#' + cfg.id);
            if (!canvas) return;
            ...
            if (chart) {
                messageElement._aiopsCharts.push(chart);
            }
        });
    }

    renderAiOpsDashboard(messageElement, data) {
        const dashboard = this.ensureAiOpsDashboardShell(messageElement);
        ...
        dashboard.innerHTML = `
            ...
            <div class="aiops-charts-grid">
                <div class="aiops-chart-card">
                    ...
                    <div class="aiops-chart-wrapper"><canvas id="aiops-cpu-chart"></canvas></div>
                </div>
                <div class="aiops-chart-card">
                    <div class="aiops-chart-title">内存使用率趋势</div>
                    <div class="aiops-chart-wrapper"><canvas id="aiops-memory-chart"></canvas></div>
                </div>
                <div class="aiops-chart-card">
                    <div class="aiops-chart-title">磁盘使用率趋势</div>
                    <div class="aiops-chart-wrapper"><canvas id="aiops-disk-chart"></canvas></div>
                </div>
            </div>`;

        this.renderAiOpsCharts(messageElement, data);
    }

    ensureAiOpsReportDivider(messageElement) {
        const wrapper = messageElement?.querySelector('.message-content-wrapper');
        if (!wrapper) return;

        if (wrapper.querySelector('.aiops-report-divider')) return;

        const divider = document.createElement('div');
        divider.className = 'aiops-report-divider';
        divider.textContent = '告警分析报告';

        const messageContent = wrapper.querySelector('.message-content');
        if (messageContent) {
            wrapper.insertBefore(divider, messageContent);
        } else {
            wrapper.appendChild(divider);
        }
    }

    async loadAndRenderAiOpsDashboard(messageElement) {
        try {
            const data = await this.fetchAiOpsDashboard(60);
            this.renderAiOpsDashboard(messageElement, data);
        } catch (error) {
            console.error('加载 AI Ops 仪表盘失败:', error);
            const dashboard = this.ensureAiOpsDashboardShell(messageElement);
            if (dashboard) {
                dashboard.innerHTML = `<div class="aiops-alert-empty">⚠️ 可视化数据加载失败: ${this.escapeHtml(error.message)}</div>`;
            }
        }
    }
    ...
    async triggerAIOps() {
        ...
        const loadingMessage = this.addLoadingMessage('平台级诊断进行中，请稍候...');
        ...
        try {
            ...
            if (loadingMessage) {
                ...
                if (messageContent) {
                    messageContent.classList.remove('de-stream-text');
                    messageContent.textContent = '抱歉，平台级诊断出现错误：' + error.message;
                }
            }
            this.showNotification('平台级诊断失败：' + error.message, 'error');
        } finally {
            this.isStreaming = false;
    ...
    showLoadingOverlay(show) {
        if (this.loadingOverlay) {
            if (show) {
                ...
                if (loadingText) loadingText.textContent = '平台级诊断进行中，请稍候...';
